...
export function LogIn()
{
    ...
    async function Submit()
    {
        ...
        if(user)
        {
            localStorage.setItem("token" , user.data.msg);
        ...
        <div className=' flex justify-center items-center h-screen bg-slate-200'>
            <div className=' w-[30rem] h-[35rem] bg-white rounded-lg border border-black'>
                ...
                <div className='flex justify-center items-center'>
                    <Button Variant='primary' Size='ul' Text='Submit' Loading={Loading} Load={<Load/>} onClick={Submit}/>
                </div>
                <div className='flex justify-center items-center mt-[2rem]'>
                    <div className='text-[0.9rem] text-slate-800 font-'>Don't Have An Account ?</div>
                    <div><a onClick={redirect} className='text-[0.9rem] font-bold'>Sign up</a></div>
                </div>
            </div>
        </div>
    </>   
